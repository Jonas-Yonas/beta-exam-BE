...
export class CurriculumRelationalRepository implements CurriculumRepository {
  ...
  async create(data: Curriculum): Promise<Curriculum> {
    ...
    const newEntity = await this.curriculumRepository.save(this.curriculumRepository.create(persistenceModel));
    return CurriculumMapper.toDomain(newEntity);
  }

  async findAllWithPagination({paginationOptions}: {paginationOptions: IPaginationOptions}): Promise<Curriculum[]> {
  ...
  async update(id: Curriculum['id'], payload: Partial<Curriculum>): Promise<Curriculum> {
    const entity = await this.curriculumRepository.findOne({
      where: {id}
    });
