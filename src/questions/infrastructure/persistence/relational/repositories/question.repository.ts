...
export class QuestionRelationalRepository implements QuestionRepository {
  ...
  async create(data: Question): Promise<Question> {
    ...
    const newEntity = await this.questionRepository.save(this.questionRepository.create(persistenceModel));
    return QuestionMapper.toDomain(newEntity);
  }

  async findAllWithPagination({paginationOptions}: {paginationOptions: IPaginationOptions}): Promise<Question[]> {
